...
def plot_keyword_clouds_by_cluster(csv_path, cluster_column="cluster", keyword_column="keywords", delimiter=",", save_dir="keyword_clouds_remove_word"):
    # Load CSV
    ...
    for cluster_id in clusters:
        ...
        for kw_str in df_cluster[keyword_column].dropna():
            all_keywords.extend([
                kw.strip() for kw in kw_str.split(delimiter)
                if kw.strip() and kw.strip() != "自分"
            ])
